...
export default function JsonBuilder() {
  ...
  const { jsonBuilder, setJsonBuilder, addField, updateField, removeField } =
    useJsonStore();
  const [template, setTemplate] = useState<string>("");
  const [inputValue, setInputValue] = useState<string>(
    JSON.stringify(jsonBuilder, null, 2),
  );
  ...
  useEffect(() => {
    setInputValue(JSON.stringify(jsonBuilder, null, 2));
  }, [jsonBuilder]);
  ...
  return (
    <div className="bg-section h-full rounded-lg p-5 flex flex-col">
      <Tabs defaultValue="basic" className="flex-grow overflow-y-auto">
                    ...
                    files: [
                      {
                        ...
                        mediaType: "application/jsonBuilder",
                      },
                    ],
                    image: "",
                    external_url: "",
        ...
        <TabsContent value="basic">
          <div className="space-y-4">
            {Object.entries(jsonBuilder).map(([key, value]) => (
              <JsonField
                parentType="object"
                key={key}
                keyName={key}
      ...
      <div className="mt-4">
        <Button onClick={() => createMetadata(jsonBuilder)} className="w-full">
          Create
        </Button>
      </div>
    </div>
